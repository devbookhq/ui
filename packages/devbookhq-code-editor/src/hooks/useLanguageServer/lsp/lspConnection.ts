...
export const createLSPConnection = (conn: MessageConnection) => {
  ...
  return {
    ...
    getHoverInfo: maybeReq(() => !!capabilities.hoverProvider, HoverRequest.type),
    ...
    getCodeLens: maybeReq(() => !!capabilities.codeLensProvider, CodeLensRequest.type),
    ...
    get syncsIncrementally() {
      let syncCapability = capabilities.textDocumentSync ?? TextDocumentSyncKind.None
...
type Params<T> = T extends ProtocolRequestType<infer P, any, any, any, any>
  ...
  ? P
  : never
type Result<T> = T extends ProtocolRequestType<any, infer R, any, any, any> ? R : never
